;       AKY music player - V1.1.
;       By Julien Névo a.k.a. Targhan/Arkos.
;       CPC PSG sending optimization trick by Madram/Overlanders.
;       December 2016.
;
;       v1.2:
;             - Added PCW / PCW 6-channel TurboSound support.
;             - Removed ":" before EQU labels, needed by RASM 2.3.9.
;             - Added an optional PLY_AKY_ADD_STOP_SOUNDS flag to add a PLY_AKY_Stop method to the sounds.
...
;             - To remove hooks, define PLY_AKY_REMOVE_HOOKS (PLY_AKY_REMOVE_HOOKS = 1).
;
;       This compiles with RASM. Please check the compatibility page on Arkos Tracker 3 website, there is a Source Converter (Disark) for ANY assembler!
;
...
;       Multi-PSG
;       ----------------------
;       This player only targets 1 PSG, as it allows some nice optimizations.
...
;       Digidrums/samples
;       ----------------------
;       Digidrums or samples are NOT supported by this version of the player. Check the PlayerAkyMultiPsg instead.
;
;       Hooks
;       ----------------------
;       Hooks are present by default.
;       - Use "PLY_AKY_REMOVE_HOOKS = 1" to remove them, to be able to call the player +0/+3/+6 to init/play/stop instead of calling the methods.
;       - Use "PLY_AKY_ADD_STOP_SOUNDS = 1" to add the Stop Sound method/hook, which is absent by default, 
;
;       Optimizations
;       ----------------------
...
;       - SIZE: Hooks for external calls (play/init) are present by default. Define PLY_AKY_REMOVE_HOOKS to remove them (PLY_AKY_REMOVE_HOOKS = 1).
...
;       - CPU:  We *could* save 3 NOPS by removing the first "jp PLY_AKY_ReadRegisterBlock" and stucking the whole code instead. But it would make
;               the whole really ugly.
;
;       -------------------------------------------------------
        ...
        IFDEF RASM_VERSION      ;To prevent BASM from interpreting this snippet.
                PLY_AKY_REPEAT_CHECK = 1
                REPEAT 7, check
                        PLY_AKY_REPEAT_CHECK = PLY_AKY_REPEAT_CHECK * check
                REND
                IF PLY_AKY_REPEAT_CHECK != 5040
        ...
        IFDEF PLY_AKY_USE_DIGIDRUMS
                FAIL 'Digidrums are not supported by this player! Use the PlayerAkyMultiPsg.'
        ENDIF
...
PLY_AKY_HardwareCounter = 0
        ...
        IFDEF PLY_AKY_HARDWARE_MSX
                PLY_AKY_HardwareCounter = PLY_AKY_HardwareCounter + 1
                PLY_AKY_HARDWARE_MSX_STANDARD = 1
                PLY_AKY_HARDWARE_FAST_PSG_ACCESS = 1            ;Indicates out (xx),a is used.
                PLY_AKY_FAST_PSG_SELECT_REGISTER = #a0
                PLY_AKY_FAST_PSG_SELECT_VALUE = #a1
        ENDIF
        ...
        IFDEF PLY_AKY_HARDWARE_PENTAGON
                ...
                PLY_AKY_HARDWARE_SPECTRUM_OR_PENTAGON = 1
        ENDIF
        IFDEF PLY_AKY_HARDWARE_PCW
                PLY_AKY_HardwareCounter = PLY_AKY_HardwareCounter + 1
                PLY_AKY_HARDWARE_FAST_PSG_ACCESS = 1
                PLY_AKY_FAST_PSG_SELECT_REGISTER = #aa
                PLY_AKY_FAST_PSG_SELECT_VALUE = #ab
        ENDIF
...
PLY_AKY_DisarkForceNonReferenceDuring3_{disarkCounter}:
        ...
        IFNDEF PLY_AKY_REMOVE_HOOKS
                ...
                IFDEF PLY_AKY_ADD_STOP_SOUNDS
                        jp PLY_AKY_Stop             ;Player + 6.
                ENDIF ;PLY_AKY_ADD_STOP_SOUNDS
        ENDIF ;PLY_AKY_REMOVE_HOOKS
...
dknr3 (void):  ld hl,1
        ...
        IFDEF PLY_AKY_ADD_STOP_SOUNDS
;Stops the sounds. It ONLY stops the sounds and is not made to pause the music.
PLY_AKY_Stop:
        IFDEF PLY_AKY_HARDWARE_MSX_STANDARD
                ld hl,#0700 + %10111111         ;On MSX, bit 7 must be 1.
        ELSE
                ld hl,#0700 + %00111111
        ENDIF
        call PLY_AKY_SendPsg
        ;Volume registers to 0 too.
        ld hl,#0800
        call PLY_AKY_SendPsg
        inc h
        call PLY_AKY_SendPsg
...
PLY_AKY_SendPsg
        ...
        IFDEF PLY_AKY_HARDWARE_SPECTRUM_OR_PENTAGON
                ...
                out (c),l       ;#bffd + value.
        ENDIF
        IFDEF PLY_AKY_HARDWARE_FAST_PSG_ACCESS
                ld a,h
                out (PLY_AKY_FAST_PSG_SELECT_REGISTER),a     ;Register.
                ld a,l
                out (PLY_AKY_FAST_PSG_SELECT_VALUE),a     ;Value.
        ENDIF
        ret
...
PLY_AKY_RRB_IS_HO_NoNoise:
        ...
        IFDEF PLY_AKY_HARDWARE_FAST_PSG_ACCESS
                ...
                out (PLY_AKY_FAST_PSG_SELECT_REGISTER),a     ;Register.
                ld a,c
                out (PLY_AKY_FAST_PSG_SELECT_VALUE),a     ;Value (volume to 16).
        ENDIF
...
PLY_AKY_RRB_NIS_HardwareOnly
        ...
        IFDEF PLY_AKY_HARDWARE_FAST_PSG_ACCESS
                ...
                out (PLY_AKY_FAST_PSG_SELECT_REGISTER),a     ;Register.
                ld a,c
                out (PLY_AKY_FAST_PSG_SELECT_VALUE),a     ;Value (16, hardware volume).
        ENDIF
